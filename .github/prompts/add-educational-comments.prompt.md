...
3. **Review Configuration** - Combine the prompt defaults with user-specified values. Interpret obvious typos using context.
...
If a configurable element is missing, use the default value.
...
```text
[user]
> /add-educational-comments #file:validation.ts Comment Detail = 1, Repetitiveness = 1, Line Number Referencing = no
```
